...
sub _TicketUpdate {
    ...
    if ( $Self->{State} =~ /$Self->{Config}->{CloseTicketRegExp}/ && !$SkipIfLocked ) {
        ...
        if ( $Self->{Config}->{CloseActionState} ne 'OLD' ) {
            $Param->{GetParam}->{'X-OTOBO-FollowUp-State'} = $Self->{Config}->{CloseActionState};
            ...
            $Param->{GetParam}->{'X-OTOBO-FollowUp-State-PendingTime'} = $DateTimeObject->ToString();
        }
...
sub _TicketCreate {
    ...
    $Param->{GetParam}->{'X-OTOBO-ArticleType'} = $Self->{Config}->{ArticleType};
